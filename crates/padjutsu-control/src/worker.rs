...
//! event loop from system input latency. macOS mouse movement has one further
//! bounded delivery worker so a blocked `CGEventPost` cannot stall planning,
//! cancellation, key actions, or scrolling.
//!
...
impl PerformerWorker {
    ...
    pub fn spawn(mut performer: Performer) -> Self {
        ...
        let mouse_generation = performer.mouse_generation();
...
struct WorkerMetrics {
    ...
    mouse_prediction_clamped: u64,
    cursor_tracking_error_axis: ValueStats,
    cursor_stalled: u64,
    cursor_recovery_warps: u64,
    cursor_stall_sequence_max: u8,
...
impl WorkerMetrics {
    fn new(enabled: bool, dropped: Arc<AtomicU64>) -> Self {
        Self {
            ...
            mouse_prediction_clamped: 0,
            cursor_tracking_error_axis: ValueStats::default(),
            cursor_stalled: 0,
            cursor_recovery_warps: 0,
            cursor_stall_sequence_max: 0,
    ...
    fn record_mouse(
        ...
        if let Some(observation) = observation {
            self.mouse_prediction_clamped +=
                u64::from(observation.prediction_clamped);
    ...
    fn maybe_report(&mut self, queue_len: usize) {
        ...
        padjutsu_metrics::metric!(
            "performer",
            "[performer-metrics] window_ms={} batches={} commands={} executions={} coalesced={} dropped={} max_batch={} queue_len={} queue_wait_us({}) queue_wait_over_4ms={} queue_wait_over_16ms={} mouse_post_us({}) mouse_post_over_4ms={} mouse_post_over_16ms={} mouse_post_over_50ms={} mouse_visual_warp_attempts={} mouse_visual_warps={} mouse_visual_warp_failures={} mouse_warp_us({}) mouse_warp_over_4ms={} mouse_warp_over_16ms={} mouse_warp_over_50ms={} mouse_interval_us({}) mouse_input_age_us({}) mouse_delta_axis_px({}) mouse_delta_change_axis_px({}) mouse_posted_delta_axis_px({}) mouse_prediction_extra_axis_px({}) mouse_prediction_clamped={} cursor_tracking_error_axis_px({}) cursor_stalled={} cursor_recovery_warps={} cursor_stall_sequence_max={} display_epoch={} display_reconfiguration_events={} mouse_posts={} mouse_commands={} cancelled_mouse_commands={} clamped_mouse_posts={} max_mouse_commands_per_post={} scroll_post_us({}) other_execution_us({})",
            ...
            self.cursor_stalled,
            self.cursor_recovery_warps,
            self.cursor_stall_sequence_max,
...
mod tests {
    ...
    fn mouse_metrics_detect_cursor_that_did_not_apply_previous_delta() {
        ...
        assert_eq!(metrics.mouse_prediction_clamped, 1);
